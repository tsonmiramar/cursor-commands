...
**MANDATORY**: Before proceeding to phase cycles (see §4), you **MUST** fill in the header of the `.ralph/Implementation_Plan.md` file. Ensure the file exists with a header template (Ralph workspace setup).

The file already contains a header template with placeholders. You must:
1. **Read the existing header** in `.ralph/Implementation_Plan.md`
...
2. **File Operation Timing**: Append to `.ralph/Implementation_Plan.md` only after complete phase cycle is presented **in chat**
...
Additionally, run a parallel **plan QA sweep** using sub-agents focused on:
- task packing violations
- classification correctness (single-step vs multi-step)
- template/format rigidity risks
- explicit user requirement compliance (Context Synthesis integration)

If any of these checks requires repo evidence generation (running commands/tests or inspecting build behavior), perform it inside an isolated `git worktree` per `.ralph/guides/Subagents_Worktree_Protocol_Guide.md`.
...
Update `.ralph/Implementation_Plan.md` with revised agent assignments:
...
Update `.ralph/Implementation_Plan.md` with enhanced dependency notations:
...
- Direct User to review complete structured plan in `.ralph/Implementation_Plan.md`
...
- Present options consistent with `.cursor/commands/ralph-initiate-setup-prd.md` §2 (exact user-review prompt and three branches).
  - **Plan looks good** → Emit `prd.json`, then the user may run `/ralph-orchestrate`.
  - **Modifications needed** → Apply changes and re-present options
  - **Systematic Review requested** → Proceed to `.ralph/guides/Project_Breakdown_Review_Guide.md`
