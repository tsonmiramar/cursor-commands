...
When executing the analytical framework for a given task, use parallel sub-agents mapped to the analysis categories:
- scope & execution feasibility
- classification correctness (single-step vs multi-step)
- Context Synthesis / user requirement integration
- output clarity, dependency sanity, and spec completeness
Each sub-agent returns normalized findings; integrate them into the task’s consolidated “Analysis Results” output.

If any sub-check requires repo evidence generation (running tests/builds or inspecting build behavior), do it inside isolated `git worktree` environments per `.ralph/guides/Subagents_Worktree_Protocol_Guide.md`.
...
2. **State Setup Phase is complete** — direct the user to run `/ralph-orchestrate` when ready to execute (after `prd.json` exists if using the Ralph setup + PRD flow)
